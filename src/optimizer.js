...
export const ADAMW_DEFAULTS = Object.freeze({
  learning_rate: 0.001,
  weight_decay: 0.004,
  beta_1: 0.9,
  beta_2: 0.999,
...
const ALLOWED_CONFIG_KEYS = new Set([
  ...Object.keys(ADAMW_DEFAULTS),
  ...Object.keys(CONFIG_ALIASES),
]);

function normalizeGradientsInput(variableGradients) {
...
function isTensorDisposed(tensor) {
  return tensor && tensor.isDisposedInternal === true;
}

function tensorShapeEquals(left, right) {
...
function asFloatTensor(value, argName) {
...
function canonicalizeConfig(config) {
  ...
  for (const [key, value] of Object.entries(config || {})) {
    ...
    const canonicalKey = CONFIG_ALIASES[key] ?? key;
    canonical[canonicalKey] = value;
  }
  return canonical;
}

export function normalizeAdamWConfig(config = {}) {
...
function validateStateTensor(tensor, referenceShape, name) {
...
export function createAdamWState(variable, config = {}) {
  const normalized = normalizeAdamWConfig(config);
...
export function getAdamWOptimizerIterations(internalIterations, config = {}) {
  const normalized = normalizeAdamWConfig(config);
...
function clipByNorm(values, clipnorm) {
...
function clipByGlobalNorm(values, clipnorm) {
  const clipNorm = tf.scalar(clipnorm, 'float32');
...
function clipGradient(values, config) {
  if (config.clipnorm != null && config.clipnorm > 0) {
    return clipByNorm(values, config.clipnorm);
  }
  if (config.global_clipnorm != null && config.global_clipnorm > 0) {
    return clipByGlobalNorm(values, config.global_clipnorm);
  }
...
function clipGradients(gradients, config) {
  if (gradients.length === 0) {
    return gradients;
  }

  if (config.clipnorm != null && config.clipnorm > 0) {
    return gradients.map((gradient) => clipByNorm(gradient, config.clipnorm));
  }
...
function createSlotVariable(referenceVariable) {
  return tf.tidy(() => tf.zerosLike(referenceVariable).variable(false));
}

function resolveState(state, variableTensor, config) {
  if (!state) {
    throw new Error('State is required. Use `createAdamWState(variable, config)` for the first step.');
  }
  ...
  validateStateTensor(state.momentum, variableTensor.shape, 'momentum');
  validateStateTensor(state.velocity, variableTensor.shape, 'velocity');
  ...
  if (config.amsgrad) {
    ...
    validateStateTensor(state.velocityHat, variableTensor.shape, 'velocityHat');
  ...
  if (config.gradient_accumulation_steps) {
    ...
    validateStateTensor(state.gradientAccumulator, variableTensor.shape, 'gradientAccumulator');
  ...
  if (config.use_ema) {
    ...
    validateStateTensor(state.ema, variableTensor.shape, 'ema');
    ema = state.ema;
  }
...
export function adamWStep(variable, gradient, state = null, config = {}) {
  const normalized = normalizeAdamWConfig(config);
  ...
  const activeState = state ?? createAdamWState(variableTensor, normalized);
  const resolvedState = resolveState(activeState, variableTensor, normalized);
  const nextState = tf.tidy(() => {
    ...
    if (shouldUpdate) {
      grad = clipGradient(grad, normalized);
...
export function disposeAdamWState(state) {
  if (!state) {
    return;
  }
...
export class AdamWOptimizer extends tf.Optimizer {
  ...
  constructor(options = {}) {
    super();
    this.config = normalizeAdamWConfig(options);
  ...
  applyGradients(variableGradients) {
    ...
    tf.tidy(() => {
      ...
      for (const name of gradientNames) {
        ...
        records.push({ name, variable, slots, gradient });
      }

      if (shouldUpdate && records.length > 0) {
        const clipped = clipGradients(records.map((record) => record.gradient), this.config);
...
export function AdamW(options = {}) {
  return new AdamWOptimizer(options);
}

tf.serialization.registerClass(AdamWOptimizer);
